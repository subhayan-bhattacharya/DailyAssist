...
import pytest
from app import app, get_user_context
...
def test_create_a_new_reminder_normal_use_case(
    reminders_model, reminders, new_reminder_request, client, mocker
):
    ...
    reminder_from_db = dynamo_backend.DynamoBackend.get_a_reminder_gsi(
        user_id="local_test_user", reminder_title=new_reminder_request["reminder_title"]
    )
    assert len(list(reminder_from_db)) == 0

    response = client.post("/reminders", json=new_reminder_request)
    returned_body = response.json()
    ...
    assert call_args[0][0] == "local_test_user"  # username
    assert (
...
def test_share_a_reminder(
    reminders_model, reminders, new_reminder_request, new_reminder, client, mocker
):
    ...
    response = client.post(
        f"/reminders/{reminder_id}/share", json={"username": "local_test_user_2"}
    )
    returned_body = response.json()
    ...
    reminder_from_db_user_2 = list(
        dynamo_backend.DynamoBackend.get_a_reminder_gsi(
            user_id="local_test_user_2", reminder_title=new_reminder_request["reminder_title"]
        )
    )
    ...
    mocked_send_confirmation.assert_called_once()
    call_args = mocked_send_confirmation.call_args
    assert call_args[0][0] == "local_test_user"  # original_user
    assert "local_test_user_2" in call_args[0][1]  # username_to_be_shared_with in message
    assert "Test reminder" in call_args[0][1]  # reminder_description in message
    assert "Reminder shared with user: local_test_user_2" in call_args[0][1]


def test_view_list_of_all_reminders_for_a_user(
    reminders_model, reminders, new_reminder, client, mocker
):
...
def test_view_list_of_reminders_filtered_by_tags(
    ...
    dynamo_backend.DynamoBackend.create_a_new_reminder(reminder_3)
    response = client.get("/reminders?tag=dummy")
    returned_body = response.json()
    assert response.status_code == 200
    reminder_ids_returned = [reminder["reminder_id"] for reminder in returned_body]
...
def test_get_remider_tags(reminders_model, reminders, new_reminder, client, mocker):
...
def test_get_details_about_a_reminder(reminders_model, reminders, new_reminder, client, mocker):
    ...
    dynamo_backend.DynamoBackend.create_a_new_reminder(reminder_1)
    response = client.get("/reminders/abc")
    returned_body = response.json()
    assert response.status_code == 200
...
def test_delete_a_reminder(reminders_model, reminders, new_reminder, client, mocker):
    ...
    dynamo_backend.DynamoBackend.create_a_new_reminder(reminder_1)
    response = client.delete("/reminders/abc")
    returned_body = response.json()
    assert response.status_code == 200
...
def test_updating_a_reminder(reminders_model, reminders, new_reminder, client, mocker):
...
def test_updating_nonexistent_reminder(reminders_model, reminders, client, mocker):
    ...
    response = client.put(
        "/reminders/nonexistent", json=changed_reminder
    )

    returned_body = response.json()
    assert response.status_code == 400
    assert "Could not update reminder nonexistent!!" in returned_body["detail"]["message"]
    assert "No such reminder with id: nonexistent" in returned_body["detail"]["error"]
...
def test_updating_reminder_with_invalid_json(
    ...
    response = client.put("/reminders/abc", data=invalid_json, headers={"Content-Type": "application/json"})

    returned_body = response.json()
    assert response.status_code == 422  # FastAPI returns 422 for JSON parse errors
    assert "detail" in returned_body  # FastAPI validation error format
...
def test_updating_reminder_with_invalid_date_format(
    ...
    returned_body = response.json()
    assert response.status_code == 400
    assert "Could not update reminder abc!!" in returned_body["detail"]["message"]
...
def test_updating_reminder_tags(reminders_model, reminders, new_reminder, client, mocker):
...
def test_updating_reminder_with_malformed_request_body(
    ...
    response = client.put("/reminders/abc", data="not-json-at-all", headers={"Content-Type": "application/json"})
...
def test_updating_reminder_with_empty_tags_list(
    ...
    # DynamoDB doesn't support empty sets, so empty tags become None
    assert reminder_from_db[0].reminder_tags is None or len(reminder_from_db[0].reminder_tags) == 0
...
def test_updating_reminder_with_once_frequency_no_expiration(
    ...
    assert response.status_code == 400
    assert "Could not update reminder abc!!" in returned_body["detail"]["message"]
    assert "Expiration date required for one-time reminders" in returned_body["detail"]["error"]
